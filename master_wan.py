...
# Recibe el NOMBRE del host como string (el mismo literal pasado a addHost) y
# deriva la interfaz por convencion '<nombre>-eth0', como en los labs (s3/s7).
def dhclient_cmd(name, extra='-v -1'):
    lease = f'/var/lib/dhcp/dhclient-{name}.leases'
    pid = f'/run/dhclient-{name}.pid'

    return f'dhclient -4 {extra} -lf {lease} -pf {pid} {name}-eth0'
WAN = [
    ('a2', '10.99.1.1/30', 'r_a1-eth3', '10.99.1.2/30', 10),
    ('b1', '10.99.2.1/30', 'r_a1-eth4', '10.99.2.2/30', 10),
    ('b2', '10.99.3.1/30', 'r_a1-eth5', '10.99.3.2/30', 20),
]
...
def harden_rp_filter(sites):
    ...
    # r_a1 DESCARTA en la IDA los paquetes cuyo origen no es enrutable de vuelta:
    # los servidores originan desde su IP de infra 172.16.1.x (que r_a1 no sabe
...
def main():
    ...
    for s in sites:
        s.configure()
    r = a1.border_router
    r.cmd('ip route replace 10.2.0.0/16 via 10.99.1.2 dev r_a1-eth3')
    r.cmd('ip route replace 10.3.0.0/16 via 10.99.2.2 dev r_a1-eth4')
    r.cmd('ip route replace 10.4.0.0/16 via 10.99.3.2 dev r_a1-eth5')
    a2.gateway.cmd('ip route replace default via 10.99.1.1 dev r_a2-eth1')
    b1.gateway.cmd('ip route replace default via 10.99.2.1 dev r_b1-eth1')
    b2.gateway.cmd('ip route replace default via 10.99.3.1 dev r_b2-eth1')
